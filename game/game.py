...
def update():
    ...
    bullet_duration = abs(ship.y - (-50)) / HEIGHT
    if not bullet.visible:
        bullet.pos = (ship.x, ship.y - 50)
        bullet.visible = True
        animate(bullet, tween='linear', duration=bullet_duration, y=-50, on_finished=lambda: reset_bullet(bullet))
    if not bullet_2.visible and timer > 2:
        bullet_2.pos = (ship.x, ship.y - 50)
        bullet_2.visible = True
        animate(bullet_2, tween='linear', duration=bullet_duration, y=-50, on_finished=lambda: reset_bullet(bullet_2))
    if not bullet_3.visible and timer % 2 == 0:
        bullet_3.pos = (ship.x, ship.y - 50)
        bullet_3.visible = True
        animate(bullet_3, tween='linear', duration=bullet_duration, y=-50, on_finished=lambda: reset_bullet(bullet_3))
